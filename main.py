...
def schedule_tournament_match(team_1_uuid: str, team_2_uuid: str, stage_label: str = "GROUP_STAGE") -> dict:
    """Creates a structured matchmaking row tracking head-to-head performance endpoints."""
    match_payload = {
        "team_1_id": team_1_uuid,
        "team_2_id": team_2_uuid,
        "match_stage": stage_label,
        "match_status": "SCHEDULED"
    }
    response = supabase_client.table("matches").insert(match_payload).execute()
    return response.data[0] if response.data else {}


def update_live_match_score(match_uuid: str, t1_score: int, t2_score: int, winner_uuid: str) -> bool:
    """Closes an active scheduled scorecard, inputs round points, and logs the advancing team."""
    update_payload = {
        "team_1_score": t1_score,
        "team_2_score": t2_score,
        "winning_team_id": winner_uuid,
        "match_status": "COMPLETED"
    }
    supabase_client.table("matches").update(update_payload).eq("match_id", match_uuid).execute()
    return True
...
async def on_ready():
    print(f"\n🤖 Production Tournament Coordination Engine Live!")
